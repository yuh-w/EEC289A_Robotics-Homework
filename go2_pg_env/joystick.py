...
def default_config() -> config_dict.ConfigDict:
    return config_dict.create(
        ...
        command_config=config_dict.create(
            ...
            stage2_track_straight_vx=[0.6, 3.2],
            stage2_track_straight_yaw=[-0.05, 0.05],
            stage2_track_curve_vx=[0.8, 2.5],
            stage2_track_curve_radius=18.25,
            stage2_track_curve_yaw_noise=[-0.03, 0.06],
            stage2_track_curve_yaw_scale=[0.85, 1.25],
            stage2_track_curve_yaw_abs=[0.05, 0.24],
            stage2_track_curve_vy=[-0.06, 0.06],
            stage2_track_recovery_vx=[0.0, 1.4],
...
class Joystick(go2_base.Go2Env):
    ...
    def _student_stage2_sampling_profile(self, current_command: jax.Array) -> tuple[jax.Array, jax.Array, jax.Array]:
        ...
        del current_command
        # 丢弃原来的 _cmd_min, 换成全向移动的 goal_min 和 goal_max
        return (
